...
time_min = '2017-05-02 12:05:00'
time_max = '2017-05-02 13:45:00'
time_min_2 = '2017-05-02 12:05:00'
time_max_2 = '2017-05-02 13:45:00'
...
sensing_naming = {'sensor_1_1': '10 message/min', 'sensor_2_1': '20 message/min', 'sensor_3_1': '40 message/min'}
...
def gen_plot(plt, data, index, line_type, num_col, nul_row, y_label=None, x_label='time(s)', title=None, marker=None,
    ...
    if isinstance(data, list):
        ...
        if data.get('x', 0) == 0:
            ...
            plt.legend(keys, ncol=int(len(keys) / 3), loc='upper right', columnspacing=1.5, labelspacing=0.0,
...
def draw_delay(is_load_from_file=False):
    ...
    p3 = plt.bar(series_3['x'], series_3['y'], width, bottom=series_2['y'])
    ...
    plt.xlabel(
        'Number of sensors per platform (on 5 platforms), with data rate 5 messages/min, \nand 1 message queues in fog tier')
    # plt.xlabel('Delay time between using 1 a/nd 2 message queues')
    plt.title('Delay time by number of sensor')
    plt.xticks(series_2['x'])
    # plt.yticks(np.arange(0, 1, 0.5))
    plt.legend((p1[0],  p3[0]), ('Sensor - Platform Delay Time', 'Platform - Cloud Delay Time'))
...
draw_delay(is_load_from_file=False)